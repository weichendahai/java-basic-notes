...
针对不同的源或目的地java.io包为程序提供了相应的输人流或输出流，这些输人输出流绝大部分都是InputStream，Outpustream，Reader或Writer的子类,例如，如果需要以字节为单位对磁盘上的文件进行读写操作，就可以分别使用InputStream和OutputStream的子类FileInputStream和FileOutputStream来创建文件输人流和文件输出流。  
